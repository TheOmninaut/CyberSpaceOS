...
if (location_of_this_script != ~/Applications/CyberSpaceOS)
    mv $PWD/CyberSpaceOS.sh ~/Applications/CyberSpaceOS
    clear
    echo "Moved script to ~/Applications/CyberSpaceOS. Please run it again from that location"
    exit
fi
...
# A Distro? Its a single file that can install an Arch Linux based operating system. The Operating System doesnt do much beyond installation & providing some rices. There will also be an ISO available with calamares installer and simple setup. The idea is to create my own distro that can be cherry picked and edited. should you want no work, download the ISO. should you want some parts but change others, get the installer and go through the text options. want to edit and change things to make a simple and easy portable setup for yourself, just get the simple and edit the text. In the end, this "OS" is really just a compilation of other softwares and works that other projects made and maintain. this also means this script can be easily backed up to be a single simple text file for full installs. it also means when things change it needs to be updated and possibly remade. This means the ISO is the safest for archiving and long term back ups, but takes the most space. 
...
#//>> Install
        ...
        #//>> Install Desktop Enviornment & Window Manager & TTY versions
...
# Insert the middle thing i want to make

function install_ttye {
doas pacman -S tmux gpm
yay -S twin
}

function install_enviornments {
...
sudo pacman -Syyuu
sudo pacman -S archlinux-keyring git base-devel grub-customizer power-profiles-daemon firewalld opendoas pipewire-jack pipewire-pulse ntfs-3g unrar p7zip
echo VULKAN-INTEL_OR_VULKAN-RADEON
...
echo -e "${CYANH}Get AUR, APPIMAGE, SNAP, & FLATPAK${RC}" && sleep 2
...
doas pacman -S fbset git base-devel pkgfile man fish curl
doas pacman -S ranger bashtop neofetch wego

}

function install_terminal_media {

doas pacman -S w3m cmus feh neovim yt-dlp newsboat ​mps-youtube
yay -S epy-ereader-git --no-confirm
doas snap install mapscii

}

function install_terminal_games {

sudo pacman -S dwarffortress
yay -S bastet 
#ninvaders pacman4console nsnake greed bsdgames 2048 nudoku
#asciicker
#steam: Cogmind, Warsim: the realm Aslona
...
yay -S brave-bin timeshift mullvad-vpn
...
doas pacman -S blender audacity kdenlive libreoffice-fresh freecad zim
yay -S vscodium-bin magicavoxel
...
doas pacman -S marble thunderbird
yay -S foxtrotgps
...
function install_plasma6_dependencies {
...
doas pacman -S steam lutris wine wine-mono wine-gecko winetricks
yay -S protontricks
...
#CHANGE To MAMKE THESE ALL APPIMAGSE
# doas pacman -Syu --needed base-devel boost catch2 cmake ffmpeg fmt git glslang libzip lz4 mbedtls ninja nlohmann-json openssl opus qt5 sdl2 zlib zstd zip unzip

doas pacman -S snes9x mupen64plus dolphin-emu dosbox fceux ppsspp
yay -S cemu-bin
doas snap install visualboyadvance-m yuzu citra epsxe pscx2 rpcs3

doas pacman -S snes9x-gtk
yay -S mupen64plus-qt jstest-gtk-git
...
# Custom bash commands
//NOT IMPLEMENTED ABOVE YET! # alias tty="plasma-session-terminate && doas systemctl stop sddm"
//CHANGED! # alias de="startplasma-wayland"
//NOT IMPLEMENTED ABOVE YET! # alias wm="some command that starts a window manager"
//NOT IMPLEMENTED ABOVE YET! # alias ttyde="doas systemctl start gpm && twin"
//NOT IMPLEMENTED ABOVE YET! # alias ttywm="Some Command to start tmux session that works like a wm
//NOT IMPLEMENTED ABOVE YET! # alias windows="some command to start a windows VM in QEMU without leaving tty and without plasma"
# alias bluetooth "doas systemctl start bluetooth"
# alias cs="./Applications/CyberSpace_OS.sh"
# alias logout="qdbus org.kde.ksmserver /KSMServer logout 0 3 3"
# alias uninstall="doas pacman -Rns"
...
//NOT IMPLEMENTED ABOVE YET! # alias ttylist="echo Web-Browser : w3m && echo Music-Player : cmus && echo File-Manager : Ranger && Task-Manager : bashtop && echo E-Reader : epy"
//IMPLEMENT INTO ALL ABOVE! # echo $sudoPW | sudo -S ----- where 0000 is the users password so it doesnt prompt for pass & ----- is the command that requires SUDO
...
#VSCode - rust-analyzer - explicit folding - nord deep - stitch(bscotch) - TODO
...
# Grub Screen https://nx2.site/grub-ascii-theme (GPD WIN 2 screen size is 160x45chars 1280x720px)
# KDE Recreations
...
#-------------------Possible Changes-----------------------------
# create a Cyberspace_OS iso for a physical backup
# check out NixOS or other reproducible arch to replace this and system as a whole 
